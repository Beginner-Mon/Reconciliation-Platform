...
### Lambda (Python 3.11) — 7 function, dùng chung 1 zip
| Function | Handler | Timeout | Memory |
|---|---|---|---|
| `api` | `api.handler.lambda_handler` | 30 | 512 |
...
### IAM
- Lambda role: S3 get/put, DynamoDB CRUD + `Query` trên `.../index/*`,
  Secrets Manager read, `states:StartExecution` + `DescribeExecution`, logs.
